...
#  
fcMatFile=topfolder+"/Dona_Analysis/FCMat/FCMat.mat"
dataFile=inputfolder+"/CAMCAN.csv"
...
def runPredictionJD(fcMatFile, dataFile, test_index, filterThr=0.01, keepEdgeFile='', iPerm=[0], SM='EmEffect', session='REST12', decon='decon', fctype='Pearson', model='Finn', outDir='', confounds=['age','intelligenceFactor']):
    ...
    edges        = data['fcMats']
   # colfile=topfolder+"/Dona_Analysis/Scripts/edgesName.mat"
    #colnames	= sio.loadmat(colfile)
    #clnm=colnames['colnames']
    print("################################")
    print("key",data.keys())
    #print("none",data['None'])
    
  #  edges=pd.DataFrame(data=edges, index=data[1:,0],columns=edges[1:,0]) 
   ...
   # print("********************************************************")
    ...
    #print("arrangeeee",np.arange(n_subs))
    ...
    if len(confounds)>0:
        ...
        for i in range(len(confounds)):
         #   print("len con",len(confounds))
          #  print("range len con",range(len(confounds)))
            corrBef.append(stats.pearsonr(conMat[:,i].T,score)[0])
     #   print("corrbef",corrBef)
    ...
    #    print(score.shape),
     #   print("reg.fit", regr.fit(conMat[train_index,:], score[train_index]))
 ...
 #   print("score", score)
  #  print("score_",score_)
   ...
   #     print(outFile)
    ...
    #    print("range 0tonedges", range(0,n_edges))450938             
    #    print("edges[train index,j]", np.squeeze(edges[train_index,1]))
     #   print("*************EGGES",edges[train_index,1])
      #  print("*************EGGES",np.squeeze(edges[train_index,1]))
   ...
   #     print("idx filtered neg",idx_filtered_neg)
    ...
    #        print(model)
         ...
         #   print('saving results')
          ...
          #  print(model)
           # print("#############################################################")
            #print("score again is",score)
            #print(score[test_index])
          ...
          #  print('xtrain isss',X_train)
           # print('#################################')
          #  print('xtrain isss',X_train)
        #    print('Xtest isss',X_test)
            rbX            = RobustScaler()
      ...
      #  print("***********elnetcof",np.abs(elnet.coef_))
       # print("************************************")
       # kaza=np.abs(elnet.coef_>0).sum()
       # print(kaza)
       # print(np.abs(elnet.coef_).sum())
        print("/////////////////////// type of x train")
        print(type(X_train))
        plt.close('all')
...
#runPredictionJD(fcMatFile, dataFile, test_index, filterThr=0.01, keepEdgeFile='', iPerm=[0], SM='EmEffect', session='REST12', decon='decon', fctype='Pearson', model='Finn', outDir=inputfolder, confounds='gender')
################################################################
################################################################
################################################################
################################################################
...
runPredictionParJD(fcMatFile, dataFile, SM='memItemEmEnh', iPerm=[0], confounds=['age','intelligenceFactor','gender','handedness'], launchSubproc=False, session='REST12',decon='decon',fctype='Pearson',model='elnet', outDir = inputfolder, filterThr=0.01, keepEdgeFile='')
