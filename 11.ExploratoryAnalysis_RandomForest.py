###
# This code is taken from Dubois et al. (2018) with minor edits to change the model from 
# Linear Regression with Elastic Net penalisation to Random Forest model
###
...
def runPredictionJD(fcMatFile, dataFile, test_index, filterThr=0.01, keepEdgeFile='', iPerm=[0], SM='EmEffect', session='REST12', decon='decon', fctype='Pearson', model='Finn', outDir='', confounds=['age','intelligenceFactor']):
    ...
    for thisPerm in iPerm: 
        ...
        if model=='Finn':
            ...
            elnet2          = RandomForestRegressor(random_state=0)
            elnet=GridSearchCV(elnet2,{'max_depth':[5,10,20,40,50]},cv=cv.split(X_train, bins_cv),n_jobs=-1)
            # TRAIN
            start_time     = time()
