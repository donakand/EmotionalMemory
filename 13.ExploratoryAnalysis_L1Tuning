###
# This code was taken from Dubois et al. (2018) with minor edits to change L1 ratio from being fixed as 0.01 to being tuned through grid search
###
...
def runPredictionJD(fcMatFile, dataFile, test_index, filterThr=0.01, keepEdgeFile='', iPerm=[0], SM='EmEffect', session='REST12', decon='decon', fctype='Pearson', model='Finn', outDir='', confounds=['age','intelligenceFactor']):
    ...
    for thisPerm in iPerm: 
        ...
        if model=='Finn':
            ...
            elnet          = ElasticNetCV(l1_ratio=[.1,.5,.7,.9,.99,1],n_alphas=50,cv=cv.split(X_train, bins_cv),max_iter=1500,tol=0.001)
            # TRAIN
            start_time     = time()
